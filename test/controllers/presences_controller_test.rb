...
class PresencesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create presence" do
    assert_difference("Presence.count") do
      post presences_url, params: { presence: { name: @presence.name, session_id: @presence.session_id, status: @presence.status, user_id: @presence.user_id } }
    end
  ...
  test "should update presence" do
    patch presence_url(@presence), params: { presence: { name: @presence.name, session_id: @presence.session_id, status: @presence.status, user_id: @presence.user_id } }
